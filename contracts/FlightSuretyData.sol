...
contract FlightSuretyData {
    ...
    uint public INSURANCE_ST_PAID = 1;
    uint public INSURANCE_ST_CLOSED = 2;
    uint public INSURANCE_ST_UNKNOWN = 0;
    uint public INSURANCE_ST_IN_PROGRESS = 1;
    ...
    */
    ...
    /**
     * @dev Add vote to airline, return reqApprove flag
     *      Can only be called from FlightSuretyApp contract
     *
     */
    ...
    function buy(address passenger, bytes32 flightKey) external requireIsOperational payable {
        insurances[flightKey] = InsuranceInfo({
            passenger: passenger,
            value: msg.value,
            status: INSURANCE_ST_IN_PROGRESS
        });
    }
    ...
    */
    function creditInsurees(bytes32 flightKey) external requireIsOperational {
        ...
        if (insurance.status == INSURANCE_ST_IN_PROGRESS) {
            ...
            insurance.status = INSURANCE_ST_PAID;
        }
    }
    ...
    */
    function closeInsurance(bytes32 flightKey) external requireIsOperational{
        InsuranceInfo insurance = insurances[flightKey];
        if (insurance.status != INSURANCE_ST_UNKNOWN) {
            insurance.status = INSURANCE_ST_CLOSED;
        }
    }
    ...
    */
    function pay(address passengerAddress) external requireIsOperational {
        uint256 balance = passengerBal[passengerAddress];
    ...
    function getAirline(address airlineAddress) public view requireIsOperational() returns (bool isExists, uint256 registeredNum, bool reqApprove, bool isFunded, uint256 votersCount, uint minVotes) {
        Airline memory airline = airlines[airlineAddress];
        return (
            airline.isExists,
